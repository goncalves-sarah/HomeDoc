import { Request, Response } from 'express';
import { getConnection, getRepository } from 'typeorm';
import Doctor from '../models/Doctor';

import bcrypt from 'bcrypt-nodejs';
import jwt from 'jwt-simple';
import doctors_view from '../views/doctors_view';
import Image from '../models/Image';

import aws from 'aws-sdk';
import fs from 'fs';
import path from 'path';
import { promisify } from 'util';

interface Img {
    originalname: string;
    size: number;
    filename: string;
    location?:string;
    path?:string;
    isAvatar?:boolean;
    doctorId?: number;
    patientId?: number;
}

const obterHash = (password: string,callback: any) => {
    bcrypt.genSalt(10, (err: Error,salt: string) => {
        bcrypt.hash(password,salt,null,(err: Error,hash:string) => callback(hash));
    });
}

const s3 = new aws.S3();

export default {

    async canUserSignUp (req: Request,res: Response) {
        const doctorRepository = getRepository(Doctor);
    
        const doctor = await doctorRepository.find({
            where: ([{crm:req.body.CRM},{email:req.body.email}])
        });
        
        if(doctor.length === 1) {
            return res.status(400).json({message:'Usuário já cadastrado!'});
        } else {
            return res.status(200).send();
        }
    },

    async signin(req: Request,res: Response) {
        const { email, password } = req.body;
    
        if(!email || !password) {
            return res.status(400).json({message:'Dados incompletos'});
        }

        const doctorRepository = getRepository(Doctor);

        try {
            const doctor = await doctorRepository.findOneOrFail({
                where: [{email:email}]
            });
            
            bcrypt.compare(password,doctor.password,(err,isMatch) => {
                if(err || !isMatch) {
                    return res.status(401).json({message:'Senha incorreta'});
                }
                
                const payload = {id: doctor.id};
                const authSecret = process.env.authSecret as string;
                
                res.status(200).json({
                    id: doctor.id,
                    userType: 'médico',
                    token: jwt.encode(payload, authSecret),
                });
            })
        } catch {
            return res.status(400).json({message:'Usuário não cadastrado!'});
        }
    },

    async index(req: Request,res: Response) {
        const doctorRepository = getRepository(Doctor);

        try {
            let doctors = await doctorRepository.find({
                relations: ['images'],
            });

            doctors.map(doctor => {
                doctor.images = doctor.images.filter(image => image.isAvatar == true);
            });

            return res.status(200).json({doctors:doctors_view.renderMany(doctors)});
        } catch {
            return res.status(400).send('Erro');
        }
    },

    async show(req: Request,res: Response) {
        const { id } = req.params;

        const doctorRepository = getRepository(Doctor);

        try {
            const doctor = await doctorRepository.findOneOrFail(id,{
                relations:['images']
            });
            return res.json({doctor:doctors_view.render(doctor)});
        } catch {
            return res.status(400).send("Not found");
        }
    },

    async updatePassword(req: Request,res: Response)  {
        const { id, newPassword, oldPassword } = req.body;

        const doctorRepository = getRepository(Doctor);

        try {
            const doctor = await doctorRepository.findOneOrFail({
                where:[{id:id}]
            });
            
            bcrypt.compare(oldPassword,doctor.password,(err,isMatch) => {
                if(err || !isMatch) {
                    return res.status(401).json({message:'Senha incorreta'});
                }
            });

            obterHash(newPassword, async (hash: string) => {
                const password = hash;
    
                doctor.password = password;    
            
                await doctorRepository.save(doctor);
    
                return res.status(201).send();
            });
        } catch {
            return res.status(400).send();
        }
    },

    async updateProfile(req: Request,res: Response) {

        let requestImages = req.files as Express.Multer.File[];
        
        const { isAvatar } = req.body;
        
        const { id } = req.params;
        
        const { 
            name,
            birthDate,
            email,
            cellphone, 
            CRM,
            specialty,
            about,
            city,
            consult_price
        } = req.body;

        const doctorRepository = getRepository(Doctor);
        
        try {
            let updatedProfile = await doctorRepository.findOneOrFail({
                where:{id},
                relations: ['images']
            });
           
            updatedProfile.name = name;
            updatedProfile.birthDate = birthDate;
            updatedProfile.email = email;
            updatedProfile.crm = CRM;
            updatedProfile.specialty = specialty;
            updatedProfile.cellphone = cellphone;
            updatedProfile.about = about;
            updatedProfile.city = city;
            updatedProfile.consult_price = consult_price;

            const imageRepository = getRepository(Image);
            
            //Adiciona novas imagens
            let images = requestImages.map((image:Img) => {
                return { 
                    url: process.env.STORAGE_TYPE === 'local' ? `${process.env.APP_URL}/pictures/${image.filename}` : image.location,
                    name:image.originalname, 
                    size: image.size,
                    key: image.filename,
                    isAvatar: image.originalname === isAvatar ? true : false,
                    doctor: updatedProfile,
                }
            });
            
            if(isAvatar.length > 0) {
                
                const profilePic = updatedProfile.images.filter(img => img.isAvatar);
                
                const { key } = profilePic[0];
                
                if(process.env.STORAGE_TYPE == 's3') {
                    s3.deleteObject({
                        Bucket: 'homedocupload',
                        Key: `${key}`,
                    }).promise();
                } else {
                    promisify(fs.unlink)(path.resolve(__dirname, '..','..','public','uploads',key));
                }
                const { id } = profilePic[0];
                await imageRepository.delete(id);
                
                updatedProfile.images = updatedProfile.images.filter(img => !img.isAvatar);
            }
            
            //AJUSTAR
            images.map(async image => {
                await imageRepository.save(image);
            });
            
            await doctorRepository.save(updatedProfile);
            return res.json(updatedProfile);
        } catch {
            return res.status(400).send("Not found");
        }
    },

    async create(req: Request,res: Response) {

        const requestImages = req.files as Express.Multer.File[];

        const { isAvatar } = req.body;
        
        const images = requestImages.map((image:Img) => {
        
            return { 
                url: process.env.STORAGE_TYPE === 'local' ? `${process.env.APP_URL}/pictures/${image.filename}` : image.location,
                name:image.originalname, 
                size: image.size,
                key: image.filename,
                isAvatar: image.originalname === isAvatar ? true : false
            }
        });

        obterHash(req.body.password, async (hash: string) => {
            const password = hash;
            
            const { 
                name,
                birthDate,
                email,
                cellphone, 
                CRM,
                specialty,
                about,
                city,
                consult_price
            } = req.body;

            const doctorRepository = getRepository(Doctor);
            
            const doctor = doctorRepository.create({
                name,
                birthDate,
                email,
                cellphone, 
                crm:CRM,
                specialty,
                about,
                city,
                consult_price,
                password,
                images
            });

            await doctorRepository.save(doctor);

            return res.status(201).json(doctors_view.render(doctor));
        })
       
    },

    async deleteProfile(req: Request,res: Response) {
        const {id} = req.params;
        
        const doctorRepository = getRepository(Doctor);

        try {
            const doctor = await doctorRepository.findOneOrFail({
                relations:['images'],
                where:{id}
            });
            
            const { images } = doctor;

            images.map(img => {

                if(process.env.STORAGE_TYPE == 's3') {
                    s3.deleteObject({
                        Bucket: 'homedocupload',
                        Key: `${img.key}`,
                    }).promise();
                } else {
                    promisify(fs.unlink)(path.resolve(__dirname, '..','..','public','uploads',img.key));
                }
            })

            await doctorRepository.delete(doctor.id);

            return res.status(200).json({message:"Usário deletado com sucesso!"});
        } catch {
            return res.status(400).json({message:"Houve um problema, tente novamente!"});
        }
    }
}